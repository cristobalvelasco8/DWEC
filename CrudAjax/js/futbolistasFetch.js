...
const getAll = async () => {
    ...
    la API de fetch */
        let respuesta = await fetch("http://localhost:5555/Futbolistas");
        ...
        //implemntamos el try-catch de la función asíncrona para manejar éxito-error
            try {
                ...
                let options = {
                    ...
                    respuesta = await fetch("http://localhost:5555/Futbolistas", options),
            ...
            try {
                let options = {
                    ...
                    respuesta = await fetch(
                        `http://localhost:5555/Futbolistas/${e.target.id.value}`,
                        options
                    ),
                    json = await respuesta.json();
...
d.addEventListener("click", async (e) => {
    if (e.target.matches(".edit")) {
        $titulo.textContent = "Editar Futbolista";
        ...
        if (isDelete) {
            ...
            try {
                let options = {
                    ...
                    respuesta = await fetch(`http://localhost:5555/Futbolistas/${e.target.dataset.id}`,options );
                    json = await respuesta.json();
