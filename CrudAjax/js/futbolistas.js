...
lo Futbolistas que debemos de tener es nodejs, por tanto, habrá que seguir los pasos:
...
la estructura básica de cualquier petición, encapsulando todo el código en la funcion ajax para luego llamarla*/
        const ajax = (options) => {
            ...
            xhr.addEventListener("readystatechange", e => {
                ...
                if (xhr.status >= 200 && xhr.status < 300) {
                    ...
                    // operador de posicion OR por por si la propiedad statusText viene vacía
            ...
            con un operado de posicion or que si el usuario no especifica el 
            método apliquemos GET por defecto*/
            xhr.open(method || "GET", url);
            /* añadir cabecera a la petición, ya que toda petición tiene cuerpo y cabecera
                En la cabecera se indica el posicion de contenido, en este caso, espera un objeto
        ...
        const getAll = () => {
            ...
            ajax({
                url: "http://localhost:5555/Futbolistas",
                ...
                success: (respuesta) => {
                    ...
                    respuesta.forEach(el => {
                        ...
                        editar, el id, nombre y posicion, para así poder después saber el contenido de los registros 
...
//evento pra detectar envío de los datos de formulario 
            ...
            if (e.target === $form) {
                ...
                if (!e.target.id.value) {
                    ...
                    ajax({
                        ...
                        url: `http://localhost:5555/Futbolistas/${e.target.id.value}`,
                        method: "PUT",
        ...
        d.addEventListener("click", e => {
            ...
            if (e.target.matches(".edit")) {
                $titulo.textContent = "Editar Módulo de Futbolistas";
    ...
    en el formulario, para ello preguntamos si el origen coincide con la clase .delete*/
            if (e.target.matches(".delete")) {
                ...
                if (isDelete) {
                    //Delete - DELETE
                    ajax({
                        url: `http://localhost:5555/Futbolistas/${e.target.dataset.id}`,
                        method: "DELETE",
                        success: (respuesta) => location.reload(),
                        error: (err) => alert(err)
                    });
