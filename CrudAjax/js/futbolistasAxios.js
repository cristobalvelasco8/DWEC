...
lo Kanto que debemos de tener es nodejs, por tanto, habrá que seguir los pasos:
...
const getAll = async () => {
    try {
        let respuesta = await axios.get("http://localhost:5555/Futbolistas"),
    ...
    if (e.target === $form) {
        ...
        if (!e.target.id.value) {
            ...
            try {
                ...
                let respuesta = await axios("http://localhost:5555/Futbolistas", options);
            ...
            try {
                let options = {
                    ...
                    let respuesta = await axios(`http://localhost:5555/Futbolistas/${e.target.id.value}`,options );
                    let json = await respuesta.data;
        ...
        if (isDelete) {
            ...
            try {
                let options = {
                    ...
                    let respuesta = await axios(`${e.target.dataset.id}`,options);
                    let json = await respuesta.data;
